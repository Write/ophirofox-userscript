...
read -r -d '' MEDIAPART_BLOCK << 'JSBLOCK'
  async function createMediapartLink(AUTH_URL_MEDIAPART, name) {
      const span = document.createElement("span");
      span.textContent = "Lire avec " + name;
      const a = document.createElement("a");
      a.href = new URL("licence", "https://" + AUTH_URL_MEDIAPART);
      a.appendChild(span);
      return a;
  }

  function findMediapartPremiumBanner() {
  ...
  async function handleMediapartMirror(config) {
      ...
      let articlePath = await GM.getValue("ophirofox_mediapart_article", null);
      ...
      if (isNotConnected) {
          ...
          await GM.deleteValue("ophirofox_mediapart_article");
      }
  }

  async function handleMediapart(config) {
      const reserve = findMediapartPremiumBanner();
      if (!reserve?.length) return;
      await GM.setValue("ophirofox_mediapart_article", new URL(window.location).pathname);
      for (const balise of reserve) {
          balise.appendChild(await createMediapartLink(config.AUTH_URL_MEDIAPART, config.name));
      }
  }

  async function mediapartOnLoad() {
...
read -r -d '' ARRETSURIMAGES_BLOCK << 'JSBLOCK'
  ...
  function asiLinkAlreadyPresent() {
  ...
  async function handleArretSurImagesMirror() {
      const checkAndRedirect = async () => {
          const currentPage = new URL(window.location);
          if (currentPage.pathname === "/autologin.php") return;
          ...
          const connected = isAsiConnected();

          if (currentPage.pathname.startsWith("/articles/")) {
              if (!connected) {
                  if (loginTs && Date.now() - loginTs < 30000) {
                      await GM.deleteValue("ophirofox_arretsurimages_article");
                      await GM.deleteValue("ophirofox_asi_login_ts");
                      return;
                  }
  ...
  async function handleArretSurImages(config) {
      ...
      function tryInject() {
          if (asiLinkAlreadyPresent()) return true;
          const reserve = findAsiPremiumBanner();
          if (!reserve?.length) return false;
          GM.setValue("ophirofox_arretsurimages_article", new URL(window.location).pathname);
          for (const balise of reserve) {
              balise.parentElement.appendChild(createAsiLink());
          }
          return true;
      }
  ...
  function watchForAsiSpaNavigation(config) {
  ...
  async function asiOnLoad() {
      const config = await getOphirofoxConfig();
      if (!config.AUTH_URL_ARRETSURIMAGES) return;
      const currentPage = new URL(window.location);
      if (currentPage.host == config.AUTH_URL_ARRETSURIMAGES) {
          ...
          watchForAsiSpaNavigation(config);
      }
  }

  asiOnLoad().catch(console.error);
JSBLOCK

read -r -d '' ALTERNATIVESECONOMIQUES_BLOCK << 'JSBLOCK'
  const BNF_ALTERNATIVESECONOMIQUES_LOGIN_URL = "https://bnf.idm.oclc.org/login?url=https://www.alternatives-economiques.fr";

  function createAeLink() {
      const span = document.createElement("span");
      span.textContent = "Lire avec BNF";
      span.className = "sub-stamp-component etiquette ophirofox-europresse";
      const a = document.createElement("a");
  ...
  function findAePremiumBanner() {
      const paywall = document.querySelector("iframe#temp-paywall");
      if (!paywall) return [];
      return [paywall];
  }

  function aeLinkAlreadyPresent() {
      return !!document.querySelector("a.ophirofox-europresse");
  }
  ...
  async function handleAlternativesEconomiquesMirror(config) {
      ...
      if (!articlePath) return;
      await GM.deleteValue("ophirofox_alternativeseconomiques_article");
      window.location.pathname = articlePath;
  }

  async function handleAlternativesEconomiques(config) {
      if (aeLinkAlreadyPresent()) return;
      const reserve = findAePremiumBanner();
      if (!reserve?.length) return;
      await GM.setValue("ophirofox_alternativeseconomiques_article", new URL(window.location).pathname);
      for (const balise of reserve) {
          const link = createAeLink();
          balise.parentElement.insertBefore(link, balise);
      }
  }

  function watchForAePaywall(config) {
  ...
  async function aeOnLoad() {
      const config = await getOphirofoxConfig();
      if (!config.AUTH_URL_ALTERNATIVESECONOMIQUES) return;
      const currentPage = new URL(window.location);
      if (currentPage.host == config.AUTH_URL_ALTERNATIVESECONOMIQUES) {
          handleAlternativesEconomiquesMirror(config);
      } else {
          watchForAePaywall(config);
      }
  }

  aeOnLoad().catch(console.error);
JSBLOCK
    ...
    if [[ $matches != *"config.js" ]]; then
      while IFS= read -r js_file; do
        if [[ $js_file == *"mediapart.js" ]]; then
          js_str="$MEDIAPART_BLOCK"
        elif [[ $js_file == *"arret-sur-images.js" ]]; then
          js_str="$ARRETSURIMAGES_BLOCK"
        elif [[ $js_file == *"alternatives-economiques.js" ]]; then
          js_str="$ALTERNATIVESECONOMIQUES_BLOCK"
